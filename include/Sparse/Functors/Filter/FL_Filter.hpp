...
namespace FIPOPT::Sparse
{
    ...
    struct FL_filter
    {
        using Objective_Barrier = barrier<Derived>;
        ...
        FL_filter(FL_filter<Derived> &&F_old,
                  Objective_Barrier &phi,
                  const MatrixBase<dVec> &x,
                  const MatrixBase<dVec> &p):  x_(x),
                                                                p_(p),
                                                                phi_(phi),
                                                                P_(F_old.P_),
                                                                filter_set_(F_old.filter_set_)
                {
                    small_search_direction_ = dVec(p.array().abs() * ((1. + x.array()).inverse())).template lpNorm<Eigen::Infinity>() < (10 * 1e-4);
                }
        ...
        {
            return phi_.Eval_cE(dVec(x_ + alpha * p_)).norm();
        }
        ...
        inline void Update_Direction(const MatrixBase<dVec> &x, const MatrixBase<dVec> &p)
        {
            x_ = x;
            p_ = p;
            small_search_direction_ = dVec(p.array().abs() * ((1. + x.array()).inverse())).template lpNorm<Eigen::Infinity>() < (10 * 1e-6);
        }
    ...
    FL_filter(barrier<Derived> &phi,
              ...
              const MatrixBase<Eigen::Matrix<double, Nx, 1>> &p) -> FL_filter<Derived>;
}
