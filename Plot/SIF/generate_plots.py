...
if __name__ == '__main__':
    ...
    for dirname in os.listdir(baseFolder):
        if dirname.endswith('.SIF') and not dirname.startswith('HS108'):
            fPath = baseFolder+ dirname + "/"
            SIF_folders.append(baseFolder + dirname + "/")
            plot_single_SIF(baseFolder + dirname + "/")

    # pool = mp.Pool(processes=6)

    # pool.map(plot_single_SIF, SIF_folders)
